...
namespace UniRx
{
    public sealed class MainThreadDispatcher : MonoBehaviour
    {
        ...
        public static void SendStartCoroutine(IEnumerator routine)
        {
            if (mainThreadToken != null)
            {
                DispatcherStartCoroutine(routine);
            }
            else
            {
#if UNITY_EDITOR
                ...
                if (!isQuitting && !object.ReferenceEquals(dispatcher, null))
                {
                    dispatcher.queueWorker.Enqueue(() =>
                    {
                        ...
                        if (distpacher2 != null)
                        {
                            DispatcherStartCoroutine(routine);//uFrame_kbe: dispatcher2.StartCoroutine(routine);
                        }
                    });
                }
            }
        ...
        public static Coroutine DispatcherStartCoroutine(IEnumerator routine)
        {
#if UNITY_EDITOR
            ...
            if (dispatcher != null)
            {
                return dispatcher.StartCoroutine(routine);//uFrame_kbe: dispatcher.StartCoroutine(routine);
            }
            else
            {
                return null;
